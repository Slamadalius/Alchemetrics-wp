...
function theme_scripts(){
    ...
    if ( !is_admin() ) {
		wp_register_script('jquery', ASSETS_URL . '/scripts/jquery-3.2.1.min.js', array(), false, '3.2.1', true);
		wp_register_script('maps', "https://maps.googleapis.com/maps/api/js?key=AIzaSyAIkSWsVHsnNvqwNc7NEGp631kz0nqPkag&callback=initMap", false, '2.1.1', true);
        wp_register_script('js_main', ASSETS_URL . '/scripts/app.js', array('jquery'), '1.0', true);
...
function theme_stylesheets(){
	...
	$styles_path = ASSETS_URL . '/style.css';

	if( $styles_path ) {
		wp_register_style('flat-icons', ASSETS_URL . '/font/flaticon.css', array(), false, 'all');
		wp_register_style('font-icons', ASSETS_URL . '/font/icon-fonts.css', array(), false, 'all');
		wp_register_style('font', "https://fonts.googleapis.com/css?family=Lato:100,300,400,700,900", array(), false, 'all');
		wp_register_style('main-css', ASSETS_URL . '/style.css', array('flat-icons', 'font-icons', 'font'), false, 'all');
...
function register_theme_menus() {
	...
	register_nav_menus(array( 
        ...
        'blog-navigation' => __( 'Blog Navigation' ) 
    ));
}
